...
public class ShortWeaponTest {
    ...
    private ShortWeapon shortWeapon;
    ...
    public void setUp(){
        ...
        shortWeapon = new ShortWeapon( "长枪", 3, new RandomGenerator( random ));
        shortWeapon.setFeature( new IceFeature() );
    }
    ...
    public void throw_exception_when_knight_use_long_weapon_feature() throws Exception{
        ...
        shortWeapon.useWeapon( knight );
        when( random.nextInt( Constant.RANDOM_MAX ) ).thenReturn( Constant.DAMAGE_TYPE_TWICE_CAN_NOT_ATTACK_ONCE - 1 );
    }

    @Test
    public void not_throw_exception_when_assassin_use_long_weapon_feature(){
        Assassin assassin = new Assassin( "张三", 20, 8 );

        try {
            shortWeapon.useWeapon( assassin );
            when( random.nextInt( Constant.RANDOM_MAX ) ).thenReturn( Constant.DAMAGE_TYPE_TWICE_CAN_NOT_ATTACK_ONCE - 1 );
